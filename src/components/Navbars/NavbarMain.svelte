<script>
  ...
  import {
    ...
    projectsInteraction,
    projectsUX,
  } from "../../assets/scripts/dataProjects";
...
<div
  class=" top-0 fixed w-full h-[64px] z-2 {menuScrollSection
    ? 'bg-transparent'
    : 'bg-whiteP '}"
></div>

<nav
  class=" {menuScrollSection
    ? isMenuRevealed
      ? ' text-blackP  '
      : 'text-whiteP '
    : 'text-blackP'}   top-0 fixed z-50 w-full py-3"
>
  <div
    class="container px-4 lg:px-10 mx-auto flex items-center justify-between max-w-none"
  >
    <div class=" btn relative flex">
      <a
        use:link
        class=" text-sm font-inter font-medium leading-relaxed flex mr-4 py-2 whitespace-nowrap lowercase"
        href="/"
        on:mouseenter={startBoxAnimation}
        on:mouseleave={stopBoxAnimation}
      >
        <div class="wrapper-logo mr-3">
          <div
            class="boxes {menuScrollSection
              ? isMenuRevealed
                ? ' blackPath'
                : 'whitePath'
              : ' blackPath'}"
...
<!-- svg overlay -->
<div
  class="menu-mobile overlay pointer-events-none gradientPath block lg:hidden"
>
...
<div class="  menu !z-[3] text-blackP hidden" bind:this={menu}>
  <div class="primary-menu w-full bottom-[56px] fixed overflow-y-auto">
    <div class="menu-container flex py-10 px-10">
      <div class="font-inter font-medium uppercase text-xs my-4">
        <h2>Interactive Interactions</h2>
      </div>
      <div
        class="wrapper text-bold font-medium flex font-newsreader text-2xl mx-3"
      >
        {#each projectsInteraction as value}
          <div class=" flex flex-row btn menu-item mb-[10px] pb-[10px]">
            <span
              class="   font-inter font-semibold text-xs mx-3 !leading-[1.5rem]"
            >
              <p>
                {value.number.length === 1 ? `0${value.number}` : value.number}
              </p></span
            >
            <a class=" btn-u" use:link on:click={revealMenu} href={value.link}>
              {value.name}</a
            >
          </div>
        {/each}
...
<!-- footer menu mobile-->
<div
  class="  menu-foot text-blackP bottom-0 fixed z-3 w-full flex flex-wrap items-center justify-between px-2 py-4 block lg:hidden"
  bind:this={menuFoot}
>
